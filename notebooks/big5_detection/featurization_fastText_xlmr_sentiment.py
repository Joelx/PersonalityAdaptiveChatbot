...
def featurization_fastText_xlmr_sentiment(train_df, test_df, embedding_type):
    ...
    if embedding_type == 'tfidf':
        ...
        glove = fasttext.load_model(model_path)
        print("fastText model load time took --- %s seconds ---" % (time.time() - start_time))
        ...
        start_time = time.time()
        train_glove = fasttext_embeddings(train_df, idf_dict, glove)
        test_glove = fasttext_embeddings(test_df, idf_dict, glove)
        print("fastText Framework took --- %s seconds ---" % (time.time() - start_time))

        normalizer_glove = MinMaxScaler()
        train_glove = normalizer_glove.fit_transform(train_glove)
        test_glove = normalizer_glove.transform(test_glove)
        
        #with open("big5_normalized_idf_fasttext_vectorizer.pkl", "wb") as f:
        #    pickle.dump(normalizer_glove, f)
    ...
    #with open("big5_normalized_featurizer.pkl", "wb") as f:
    #    pickle.dump(normalizer_glove, f)
